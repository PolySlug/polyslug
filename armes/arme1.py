...
class Arme1(Arme) :
    ...
    def tirer(self, position, vecteur) :
        ...
        if pygame.time.get_ticks() - self.dernierTir > self.delai :
            ...
            return [ProjectileDroitRouge((position[0]+30*((2*self.angle)/(1+self.angle*self.angle)), position[1]+20+30*((1-self.angle*self.angle)/(1+self.angle*self.angle))), vecteur)] #Le tuple position est modifie afin de parametrer un cercle de rayon r=30px selon l'angle.
